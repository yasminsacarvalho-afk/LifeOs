...
// Replace the button for gestao with analise
...
// Replace the entire gestao block with analise block
// We need to match `{activeTab === "gestao" && (` down to `)}`
// Using regex is tricky here because it spans 300+ lines.
// I'll just write a script to find the start and end of that block.
...
if (startIndex !== -1) {
    ...
    if (endIndex !== -1) {
        // Also need to consume the final parenthesis `)`
        ...
        const newBlock = `{activeTab === "analise" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-5xl mx-auto">
            ...
            <div className="flex justify-end">
               <button 
                 onClick={() => {
                   // Save to localStorage using the date as key
                   const payload = {
...
// Ensure React is imported if missing since we use React.useEffect
