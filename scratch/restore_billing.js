...
function BillingPage() {
  ...
  const financialPeriodDates = useMemo(() => {
    ...
    const systemStart = new Date("2026-06-04T00:00:00");
    
    if (currentDate < systemStart) {
      currentDate = new Date(systemStart);
    }
    ...
    <>
      ...
      <main className="px-4 md:px-8 py-6 md:py-8 space-y-6 max-w-full overflow-hidden">
        ...
        <div className="flex space-x-1 rounded-xl bg-card border border-border p-1 w-full overflow-x-auto no-scrollbar md:w-max">
          ...
          >
            <Lightbulb className="size-4 shrink-0" /> Análise Diária (Diário)
          </button>
        </div>
        ...
        {activeTab === "fechamentos" && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-4xl mx-auto">
            <div className="grid gap-4">
              {financialPeriodDates.length === 0 ? (
                <div className="p-12 text-center text-muted-foreground border border-dashed rounded-xl bg-card/40">
                  Nenhum dado encontrado para a competência atual.
                </div>
              ) : (
                financialPeriodDates.map(date => {
                  const closing = closings.find(c => c.closing_date === date);
                  const isToday = date === todayStr;

                  if (closing) {
                     // Rendering ClosingCard would require the component, which is at the bottom.
                    return (
                      <ClosingCard 
                        key={closing.id} 
                        closing={closing} 
            ...
            <div className="flex justify-end pt-4">
               <button 
                 onClick={() => {
                   ...
                   alert("Análise Diária salva com sucesso! Estes dados ficarão armazenados para geração futura de relatórios na aba Growth.");
                 }}
...
function ClosingCard({ closing, onEdit, onDelete }: { closing: any, onEdit: () => void, onDelete: () => void }) {
  ...
  if (closing.company_settlements && Array.isArray(closing.company_settlements)) {
    closing.company_settlements.forEach((s: any) => {
    ...
    <div className="p-6 rounded-2xl bg-card border border-border shadow-sm hover:shadow-md transition-all group/card relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-2 py-5 border-y border-border/50 mb-5">
            <button 
              disabled={updating}
              onClick={() => handleToggle('boleto_generated', !closing.boleto_generated)}
              className={cn("flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-all border shadow-sm", closing.boleto_generated ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-foreground hover:bg-muted")}
            >
              {closing.boleto_generated ? <CheckCircle2 className="size-4" /> : <div className="size-4 rounded-full border-2 border-current opacity-40" />}
              Boleto Gerado
            </button>
            ...
            <button 
              ...
              className={cn("flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-all border shadow-sm", closing.boleto_paid ? "bg-success text-success-foreground border-success" : "bg-background border-border text-foreground hover:bg-muted disabled:opacity-50")}
            >
              {closing.boleto_paid ? <CheckCircle2 className="size-4" /> : <div className="size-4 rounded-full border-2 border-current opacity-40" />}
              Boleto Pago
            </button>

            {closing.boleto_receipt_url ? (
               <div className="flex items-center gap-1">
                 <a href={closing.boleto_receipt_url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold bg-info/10 text-info border border-info hover:bg-info/20 transition-all">
                   <Receipt className="size-4" /> Ver Comprovante
                 </a>
                 <button onClick={removeReceipt} disabled={updating} className="p-2 text-danger hover:bg-danger/10 rounded-full transition-colors">
                   <XCircle className="size-4" />
                 </button>
               </div>
            ) : (
               <label className={cn("flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-all border border-dashed border-border cursor-pointer hover:bg-muted text-muted-foreground", uploading && "opacity-50 pointer-events-none")}>
...
`;
fs.writeFileSync('src/routes/billing.tsx', content);
